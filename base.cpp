...
enum {
	REFLEXIVO,NO_SIMETRICO,SIMETRICO, NO_SIMETRICO_CURVO
};
const double PI=3.14159265358979;
const int TCIRCULO=17;

void obtenerPantalla(int TPX, int TPY);
		...
		switch (tipo) {
			...
			case NO_SIMETRICO:
						//	float magn,dire,xaux,yaux,xaux2,yaux2;
							...
							texto(xaux-8,yaux+3,tex.c_str());
							break;
			case SIMETRICO:
							break;
			case NO_SIMETRICO_CURVO:
							float magn,dire,dire2,desfase, proporcion,xaux,yaux,xaux2,yaux2,xaux3,yaux3,xaux4,yaux4,anguloPrincipal;
							rectangularPolar(x1,y1,x2,y2,magn,dire);
							anguloPrincipal=dire2=dire;
							desfase=20; //desfase del angulo principal
							proporcion=4;// Proporción del tamaño del circulo
...
//	strcpy(ID,nombre.c_str());
   ...
   contador1+=50;
	...
	switch(tipo)
	{
	case 0:	InicialAceptacion(nombre.c_str());	
	break;
...
void estado::InicialAceptacion(const std::string& nom)
{
...
void estado::Inicial(const std::string& nom)
{
	...
	circulo(x,y,TCIRCULO);
	texto(x-65,y-15,"Inicio");
	texto(x-4*(strlen(nom.c_str())),y-8,nom.c_str());	
}
